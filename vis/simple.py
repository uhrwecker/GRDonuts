...
class Plotter():
    ...
    def _adjust_plot(self, r, xm, ym, wmin, wmax):
        pl.xlim(r[0]-xm, r[-1]+xm)
        pl.ylim(wmin-ym, wmax+ym)
        pl.xlabel('R/M')
        pl.ylabel('W')
        pl.legend()
...
class BeautyPlotter(Plotter):
    ...
    def plot(self, potential, label='', xmargin=0, ymargin=0.003,
        ...
        if fill:
            if int_l < int_r:
                pl.fill_between(r[int_l:int_r], [wmax[0] for n in r[int_l:int_r]],
                                w[int_l:int_r], color='green', alpha=0.4)
            else:
                pl.fill_between(r[int_r:int_l], [wmax[0] for n in r[int_r:int_l]],
